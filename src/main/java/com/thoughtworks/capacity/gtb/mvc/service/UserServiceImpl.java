...
public class UserServiceImpl implements UserService{
    ...
    public void createUser(User user) {
        ...
        if(username == null){throw new IllegalArgumentException("用户名不为空");}
        if(userRepository.existsUsersByUsername(username)){throw new IllegalArgumentException("用户名已存在");}
        if(password == null){throw new IllegalArgumentException("密码不为空");}
    ...
    public User validateUser(String username, String password) {
        ...
        if(validUsers.size() != 1){throw new UserNotFoundException("用户名或密码错误");}
